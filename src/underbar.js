(function() {
  ...
  _.reject = function(collection, test) {
    ...
    return _.filter(collection, function(item, i) {
      return !test(item);
    });
  };
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (arguments.length > 1) {
      _.each(array, function(item) {
        if (iterator(item) && isTrue === 0) {
          isTrue = item;
          res.push(item);
        } else if (!iterator(item) && isFalse === 0) {
          isFalse = item;
          res.push(item);
        }
      });
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    _.each(collection, function(item, key, collection) {
      if (accumulator === undefined && start === true) {
        start = false;
        accumulator = item;
      } else {
  ...
  _.extend = function(obj) {
    var args = arguments;
    return _.reduce(args, function(val, obj, key, coll) {
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      if (storage[args]) {
        ...
        storage[args] = func.apply(this, argsArr);
        return storage[args];
      }
    };
  };
